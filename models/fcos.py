...
class QuantizableFCOSHead(nn.Module):
    """
    ...
    def __init__(
        ...
        self.classification_head = QuantizableFCOSClassificationHead(
            in_channels, num_anchors, num_classes, num_convs
        )
        self.regression_head = QuantizableFCOSRegressionHead(
...
class QuantizableFCOSClassificationHead(nn.Module):
    """
...
class QuantizableFCOSRegressionHead(nn.Module):
    """
...
class QuantizableFCOS(nn.Module):
    """
    Implements FCOS.
        ...
        for index in range(num_images):
            ...
            for (
                ...
                scores_per_level = torch.sqrt(
                    torch.sigmoid(logits_per_level)
    ...
    def forward(
        ...
        if torch.jit.is_scripting():
            if not self._has_warned:
                warnings.warn(
                    "FCOS always returns a (Losses, Detections) tuple in scripting"
                )
