...
class QuantizableResNet(nn.Module):
    def __init__(
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, QuantizableBottleneck) and m.bn3.weight is not None:
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, QuantizableBasicBlock) and m.bn2.weight is not None:
    ...
    for module_name, module in model.named_children():
        if "layer" in module_name:
            for basic_block_name, block in module.named_children():
                if isinstance(block, QuantizableBasicBlock):
                    _fuse_modules(
                        block,
                ...
                elif isinstance(block, QuantizableBottleneck):
                    _fuse_modules(
                        block,
                        [
                            ["conv1", "bn1", "relu1"],
