...
def run(
    ...
    pbar = tqdm(
        dataloader, desc=s, ncols=NCOLS, bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}"
    )  # progress bar
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        if opt.task == "speed":  # speed benchmarks
            # python yolo_val.py --task speed --data coco.yaml --batch 1 --weights yolov3.pt yolov3-spp.pt...
            opt.conf_thres, opt.iou_thres, opt.save_json = 0.25, 0.45, False
            for opt.weights in weights:
                run(**vars(opt), plots=False)

        elif opt.task == "study":  # speed vs mAP benchmarks
            # python yolo_val.py --task study --data coco.yaml --iou 0.7 --weights yolov3.pt yolov3-spp.pt...
            for opt.weights in weights:
