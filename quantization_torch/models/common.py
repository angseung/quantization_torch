...
from quantization_torch.utils.datasets import exif_transpose, letterbox
...
class BottleneckReLU(nn.Module):
    # Standard bottleneck
    def __init__(
        self, c1, c2, shortcut=True, g=1, e=0.5
    ):  # ch_in, ch_out, shortcut, groups, expansion
...
class BottleneckCSPReLU(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(
        self, c1, c2, n=1, shortcut=True, g=1, e=0.5
    ):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.act = nn.ReLU()
        self.m = nn.Sequential(
            *(BottleneckReLU(c_, c_, shortcut, g, e=1.0) for _ in range(n))
        )
...
class SPPFReLU(nn.Module):
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = ConvBnReLU(c1, c_, 1, 1)
        self.cv2 = ConvBnReLU(c_ * 4, c2, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
