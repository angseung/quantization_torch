...
from quantization_torch.utils.downloads import gsutil_getsize
from quantization_torch.utils.metrics import box_iou, fitness
from quantization_torch.utils.soft_nms import soft_nms
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if use_soft:
            i = soft_nms(boxes, scores, thresh=iou_thres)  # soft NMS
        else:
...
def convert_model_to_onnx(
    ...
    dummy_input = torch.randn(size=input_size, requires_grad=True)
